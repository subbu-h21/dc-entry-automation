...
Loads `.env` (OpenRouter/ElevenLabs keys, `PORT`, `ALLOWED_ORIGINS`, `PRODUCT_LIST_PATH`/`PRODUCT_LIST_SHEET`, `DC_PIPELINE_URL`, and per-branch CRM login credentials — `HOSPET_ROAD_USERNAME`/`PASSWORD`, `SHIVAJI_CHOWK_USERNAME`/`PASSWORD`, assembled into `BRANCH_CREDENTIALS` and consumed by `routes/browser.py`'s `login()`), loads `ALL_SUPPLIERS` from `supplier_names.csv`, and hardcodes `STAFF_NAMES`. Note `STAFF_NAMES` is duplicated verbatim in `frontend/src/App.tsx` — keep both in sync if the staff list changes.
